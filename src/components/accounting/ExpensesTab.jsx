...
const StatCard = ({ label, value, icon: Icon }) => (
  <div className="rounded-lg bg-graphite-100 p-3 sm:p-4 dark:bg-graphite-700">
    <div className="flex items-center gap-2">
      <Icon className="h-4 w-4 text-graphite-500 dark:text-graphite-400" />
...
export default function ExpensesTab() {
  ...
  return (
    <div>
      ...
      {/* Helpful Hint */}
      <div className="mb-4 rounded-lg bg-rose-50 px-4 py-3 border border-rose-200 dark:border-rose-800 dark:bg-rose-900/30">
        <div className="flex items-start gap-3">
          <div className="flex-shrink-0">
            <TrendingUp className="h-5 w-5 text-rose-600" />
          </div>
          <div className="flex-1">
            <h4 className="text-sm font-semibold text-rose-900 mb-1 dark:text-rose-300">
              {t("expensesHintTitle")}
            </h4>
      ...
      <form
        ...
        className="mb-8 rounded-xl border border-graphite-200 bg-white p-6 shadow-sm dark:border-graphite-700 dark:bg-graphite-800"
      >
        <div className="mb-4 flex items-center justify-between">
          ...
          {editingId && (
            <button
              ...
              className="text-xs font-medium text-graphite-500 hover:text-graphite-700 dark:text-graphite-400 dark:hover:text-graphite-200"
            >
              {t("cancelEdit")}
            </button>
          )}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <textarea
              ...
              className="mt-1.5 block w-full resize-none rounded-lg border border-graphite-300 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 dark:border-graphite-600 dark:bg-graphite-700 dark:text-graphite-100 dark:focus:border-primary-500 dark:focus:ring-primary-500/30"
            />
          </div>
        </div>
      ...
      <div className="mb-5 rounded-xl border border-graphite-200 bg-white p-4 shadow-sm dark:border-graphite-700 dark:bg-graphite-800">
        <h3 className="mb-3 text-sm font-semibold text-graphite-900 dark:text-graphite-100">
          {t("filters")}
        </h3>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div>
            <label className="block text-xs font-medium text-graphite-700 mb-1 dark:text-graphite-300">
              {t("category")}
            </label>
            <select
              value={filterCategory}
              onChange={(e) => setFilterCategory(e.target.value)}
              className="block w-full rounded-lg border border-graphite-300 bg-white px-3 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 dark:border-graphite-600 dark:bg-graphite-700 dark:text-graphite-100 dark:focus:border-primary-500 dark:focus:ring-primary-500/30"
            >
        ...
        <div className="mb-5 rounded-xl border border-graphite-200 bg-white p-4 shadow-sm dark:border-graphite-700 dark:bg-graphite-800">
          ...
          <div className="space-y-2">
            {summary.map((item) => (
              <div
                key={item._id}
                className="flex items-center justify-between rounded-lg bg-graphite-50 px-3 py-2 dark:bg-graphite-800"
              >
                <span className="text-sm font-medium text-graphite-900 capitalize dark:text-graphite-100">
                  {item._id}
                </span>
      ...
      {/* Expenses List */}
      <div>
        <h2 className="mb-3 text-sm font-semibold text-graphite-900 dark:text-graphite-100">
          {t("expenseHistory")}
        </h2>
        {loading ? (
          <div className="flex items-center gap-2">
          ...
          <div className="divide-y divide-graphite-200 rounded-xl border border-graphite-200 bg-white dark:divide-graphite-700 dark:border-graphite-700 dark:bg-graphite-800">
            {expenses.map((expense) => (
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-graphite-900 dark:text-graphite-100">
                      {expense.amount.toFixed(2)} {expense.currency}
                    </span>
                    <span className="rounded-full bg-primary-100 px-2 py-0.5 text-xs font-medium text-primary-700 capitalize dark:bg-primary-900/30 dark:text-primary-400">
                      {expense.category}
                    </span>
                  </div>
                  {expense.description && (
                    <p className="mt-1 text-xs text-graphite-600 dark:text-graphite-400">
                      {expense.description}
                    </p>
                  )}
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="rounded-lg p-2 text-graphite-400 hover:bg-graphite-100 hover:text-primary-600 dark:text-graphite-500 dark:hover:bg-graphite-700 dark:hover:text-primary-400"
                  >
